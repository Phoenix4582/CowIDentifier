...
def scatter(x, labels, title=None, subtitle=None, filename=None, file_format="pdf"):
...
# Another version of plot function which highlights incorrect choices
def scatter_highlight(x, labels, mask, title=None, subtitle=None, filename=None, file_format="pdf", highlight_center=False):
    # Get the number of classes (number of unique labels)
    ...
    # Map the colours to different labels
    label_colours = np.array([palette[int(labels[i])] for i in range(labels.shape[0])])
    class_colours = np.array([palette[int(class_item)] for class_item in classes])
    # label_colours = np.array([palette[i] for i in range(labels.shape[0])])
    ...
    # Plot the points
    true_mask = np.where(mask == True)[0]
    false_mask = np.where(mask == False)[0]
    true_x, false_x = x[true_mask], x[false_mask]

    plt.scatter( true_x[:,0], true_x[:,1],
                lw=0, s=15,
                c=label_colours[true_mask],
                marker="o",
                alpha=1.0,
                edgecolor=None)

    plt.scatter( false_x[:,0], false_x[:,1],
                lw=0, s=25,
                c='k',
                marker="o",
                alpha=1.0,
