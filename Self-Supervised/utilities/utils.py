...
from datasets.OpenSetCows2021.OpenSetCows2021 import OpenSetCows2021
from datasets.OpenSetCows2022.OpenSetCows2022 import OpenSetCows2022
...
class Utilities:
    # Class constructor
    def __init__(self, args, lightning_mode=False):
        # Store the arguments
        self.args = args
        ...
        # check if using PyTorch Lightning
    ...
    def setupForTraining(self, args):
        ...
        elif args.model == "TripletTransformer":
            model = TripletTransformer(backbone=self.args.backbone, pretrained=True, num_classes=dataset.getNumClasses())
        else:
            print(f"Model choice: \"{args.model}\" not recognised, exiting.")
            sys.exit(1)

        # Put the model on the GPU and in training mode, ignore if using PyTorch Lightning
        if self.lightning_mode == False:
            model.cuda()
            model.train()
    ...
    def test(self, step):
        ...
        if self.args.model == 'TripletTransformer':
            run_str += " --backbone_type=Transformer"
            run_str += f" --backbone_name={self.args.backbone}"
    ...
    def selectDataset(args, train):
        ...
        elif args.dataset == "OpenSetCows2021":
            dataset = OpenSetCows2021( args.current_fold,
                                       args.folds_file,
                                       split=split,
                                       transform=True,
                                       combine=True,
                                       suppress_info=False,
                                       dataset_transforms=ds_transforms)
        elif args.dataset == "OpenSetCows2022":
            dataset = OpenSetCows2022( args.current_fold,
                                       args.folds_file,
                                       split=split,
                                       transform=True,
                                       combine=True,
